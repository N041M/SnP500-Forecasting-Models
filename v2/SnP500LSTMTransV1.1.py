"""
Enhanced Hybrid LSTM + Transformer Model for S&P 500 Prediction (V1.1)
========================================================================
This script implements an enhanced hybrid LSTM+Transformer model with all major
improvements from v1.1.2, including:

- 20+ engineered features (MA, RSI, MACD, Bollinger Bands, Stochastic, volume indicators)
- Bidirectional LSTM layers for local pattern capture (128, 64 units)
- Transformer layers for long-range dependencies (3 layers, 4 heads)
...
Architecture Flow:
Input → Bidirectional LSTM → Transformer → Dense → Dual Outputs
...
class EnhancedHybridConfig:
    ...
    @classmethod
    def load(cls, filepath: Union[str, Path]) -> 'EnhancedHybridConfig':
        filepath = Path(filepath)
...
class EnhancedHybridPredictor:
    """
    Enhanced Hybrid LSTM+Transformer model for S&P 500 prediction with custom biases

    Architecture Flow:
    Input → Bidirectional LSTM → Transformer → Dense → Dual Outputs

    Rationale:
    - Bidirectional LSTM: Captures local patterns and sequential dependencies in both directions
    - Transformer: Captures long-range dependencies and attention patterns
    - Custom biases: Growth bias, top performer weighting, sector rotation
    - Dual outputs: Price prediction + direction classification for multi-task learning
    """

    def __init__(self, config: EnhancedHybridConfig, n_features: int = 1):
        self.config = config
        ...
        logger.info("Enhanced Hybrid LSTM+Transformer Predictor initialized with custom biases")
    ...
    def prepare_data(self, features: pd.DataFrame) -> Tuple[Tuple[np.ndarray, np.ndarray, np.ndarray], ...]:
        """
        Prepare data for dual-output Hybrid training (price + direction)
        ...
        """
        logger.info("Preparing data for Hybrid LSTM+Transformer training...")
    ...
    def build_model(self):
        """
        Build enhanced Hybrid LSTM+Transformer model with dual outputs
        """
        print("\n" + "="*60)
        print("BUILDING ENHANCED HYBRID LSTM+TRANSFORMER MODEL (V1.1)")
        print("="*60)
        ...
        # ============================================
        # BIDIRECTIONAL LSTM COMPONENT (Local Pattern Capture)
        # ============================================
        x = inputs
        ...
        for i, units in enumerate(self.config.lstm_units):
            ...
            x = Dropout(self.config.dropout_rate, name=f'lstm_dropout_{i+1}')(x)

        # ============================================
        # PROJECTION TO TRANSFORMER DIMENSION
        # ============================================
        # Project LSTM output to d_model dimension
        lstm_output_dim = self.config.lstm_units[-1] * 2  # Bidirectional doubles dimension

        if lstm_output_dim != self.config.d_model:
            x = Dense(self.config.d_model, name='projection_to_transformer')(x)
        ...
        for i, units in enumerate(self.config.dense_units):
            ...
            x = Dropout(self.config.dropout_rate, name=f'dense_dropout_{i+1}')(x)
        ...
        # Create model
        model = Model(inputs=inputs, outputs=[price_output, direction_output], name='Enhanced_Hybrid_LSTM_Transformer')

        # Custom biased MSE loss function (penalizes under-predictions 2x more - from LSTM V1.1)
        ...
        model.compile(
            ...
            loss={'price_output': custom_biased_mse, 'direction_output': 'binary_crossentropy'},
        ...
        print(f"  Bidirectional LSTM layers: {len(self.config.lstm_units)}")
        print(f"  LSTM units: {self.config.lstm_units}")
        print(f"  Transformer d_model: {self.config.d_model}")
        print(f"  Transformer heads: {self.config.num_heads}")
        print(f"  Transformer layers: {self.config.num_transformer_layers}")
    ...
    def train(self, train_data: Tuple[np.ndarray, np.ndarray, np.ndarray],
        ...
        """
        Train the enhanced Hybrid model with learning rate warmup
        """
        ...
        print("\n" + "="*60)
        print("TRAINING ENHANCED HYBRID LSTM+TRANSFORMER MODEL")
        print("="*60)
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                monitor='val_price_output_loss',
                mode='min',
                filepath='best_enhanced_hybrid_model.keras',
                save_best_only=True,
                verbose=0
            ),
            lr_scheduler
    ...
    def apply_growth_bias(self, predictions: np.ndarray, features: pd.DataFrame = None) -> np.ndarray:
        """
        Apply simple growth bias to predictions (V1.1 style)
        """
    ...
    def evaluate(self, test_data: Tuple[np.ndarray, np.ndarray, np.ndarray],
        ...
        directional_accuracy = np.mean((direction_predictions.flatten() > 0.5).astype(int) == y_test_direction) * 100

        # Bias effect calculation (V1.1 style)
        ...
        metrics = {
            ...
            'Bias_Effect_%': float(bias_effect)
        }

        # Print results (V1.1 style)
        print("\n" + "="*60)
        print("HYBRID LSTM+TRANSFORMER V1.1 EVALUATION RESULTS")
        print("="*60)
    ...
    def plot_training_history(self, save_path='plots/'):
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}enhanced_hybrid_training_history.png', dpi=300, bbox_inches='tight')
        print(f"Training history plot saved to: {save_path}enhanced_hybrid_training_history.png")
        plt.close()
    ...
    def plot_results(self, predictions: np.ndarray, actual: np.ndarray,
        ...
        plt.title(f'Enhanced Hybrid LSTM+Transformer Predictions - {dataset_name} Set', fontsize=16)
        plt.xlabel('Time Steps')
        plt.ylabel('Price ($)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(f'{save_path}enhanced_hybrid_predictions_{dataset_name.lower()}.png',
                   dpi=300, bbox_inches='tight')
        print(f"Predictions plot saved to: {save_path}enhanced_hybrid_predictions_{dataset_name.lower()}.png")
        plt.close()
    ...
    def save_model(self, save_path='models/'):
        ...
        self.model.save(f'{save_path}enhanced_hybrid_model.keras')
        self.config.save(f'{save_path}enhanced_hybrid_config.json')

        print(f"Model saved to: {save_path}enhanced_hybrid_model.keras")
        print(f"Config saved to: {save_path}enhanced_hybrid_config.json")




def create_default_config(sequence_length: int = 20) -> EnhancedHybridConfig:
    """Create default configuration for enhanced Hybrid"""
    return EnhancedHybridConfig(
...
def main():
    """
    Main execution function - complete Enhanced Hybrid V1.1 training pipeline
    ...
    """
    print("\n" + "="*80)
    print("ENHANCED HYBRID LSTM+TRANSFORMER MODEL V1.1 FOR S&P 500 PREDICTION")
    print("="*80 + "\n")
    ...
    # ========================================================================
    # Derive direction labels from returns (V1.1 enhancement)
    # ========================================================================
    ...
    config.save('models/enhanced_hybrid_v1.1_config.json')

    logger.info("=" * 70)
    logger.info("📊 ENHANCED HYBRID LSTM+TRANSFORMER PREDICTOR V1.1")
    logger.info("=" * 70)
    ...
    logger.info(f"🧠 Bidirectional LSTM: {config.lstm_units}")
    logger.info(f"🧠 Transformer: {config.num_transformer_layers} layers, {config.num_heads} heads, d_model={config.d_model}")
    logger.info(f"🎯 Max Epochs: {config.epochs}")
    logger.info("=" * 70)

    # Initialize Enhanced Hybrid predictor with V1.1 architecture
    hybrid = EnhancedHybridPredictor(config, n_features=metadata['n_features'])
    ...
    # Build enhanced model with bidirectional LSTM, Transformer, and dual outputs
    hybrid.build_model()

    # Train model with learning rate warmup and custom biased loss (parameters from config)
    hybrid.train(
        (X_train, y_train, y_train_direction),
        (X_val, y_val, y_val_direction),
        warmup_epochs=10  # LR warmup for Transformer component
    )
    ...
    train_metrics, _, _ = hybrid.evaluate(
        (X_train, y_train, y_train_direction),
        None  # No features DataFrame needed
    )
    val_metrics, _, _ = hybrid.evaluate(
        (X_val, y_val, y_val_direction),
        None
    )
    test_metrics, price_predictions, y_test_actual = hybrid.evaluate(
    ...
    results = {
        ...
        'config': {
            'lstm_units': hybrid.config.lstm_units,
            'd_model': hybrid.config.d_model,
            'num_heads': hybrid.config.num_heads,
            'num_transformer_layers': hybrid.config.num_transformer_layers,
            'dff': hybrid.config.dff,
            'dropout_rate': hybrid.config.dropout_rate,
            'learning_rate': hybrid.config.learning_rate,
            'growth_bias': hybrid.config.growth_bias,
            'bidirectional_lstm': True,
            'training_time': hybrid.training_time
        }
    }

    with open('results/enhanced_hybrid_v1.1_results.json', 'w') as f:
        json.dump(results, f, indent=4)

    print("\nResults saved to: results/enhanced_hybrid_v1.1_results.json")
    ...
    hybrid.plot_results(price_predictions, y_test_actual, save_path='plots/')

    # Save model
    hybrid.save_model(save_path='models/')

    print("\n" + "="*80)
    print("ENHANCED HYBRID V1.1 PIPELINE COMPLETE")
    print("="*80)
    print("\nGenerated files:")
    print("  - models/enhanced_hybrid_v1.1_model.h5")
    print("  - models/enhanced_hybrid_v1.1_config.json")
    print("  - results/enhanced_hybrid_v1.1_results.json")
    print("  - plots/enhanced_hybrid_v1.1_training_history.png")
    print("  - plots/enhanced_hybrid_v1.1_predictions_test.png")
    print("\n" + "="*80 + "\n")

    return hybrid, test_metrics
