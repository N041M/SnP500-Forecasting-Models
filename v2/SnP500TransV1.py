"""
Transformer Model for S&P 500 Prediction
=========================================
This script implements a Transformer neural network
...
class TransformerPredictor:
    """
    Transformer model for time series prediction of S&P 500
    """
    ...
    def build_model(self):
        ...
        print(f"Feed-forward dimension: {self.dff}")
        print("="*60 + "\n")
    ...
    def train(self, X_train, y_train, X_val, y_val,
              ...
              verbose=1):
        """
        Train the Transformer model with learning rate warmup
    ...
    def plot_training_history(self, save_path='plots/'):
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}transformer_training_history.png', dpi=300, bbox_inches='tight')
        print(f"Training history plot saved to: {save_path}transformer_training_history.png")
        plt.close()
    ...
    def plot_predictions(self, y_actual, y_pred, dates=None, dataset_name='Test', save_path='plots/'):
        ...
        axes[0].set_title(f'Transformer Predictions vs Actual - {dataset_name} Set',
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}transformer_predictions_{dataset_name.lower()}.png',
                   dpi=300, bbox_inches='tight')
        print(f"Predictions plot saved to: {save_path}transformer_predictions_{dataset_name.lower()}.png")
        plt.close()
    ...
    def save_model(self, save_path='models/'):
        ...
        # Save Keras model
        self.model.save(f'{save_path}transformer_model.h5')
        ...
        with open(f'{save_path}transformer_config.json', 'w') as f:
            json.dump(config, f, indent=4)

        print(f"Model saved to: {save_path}transformer_model.h5")
        print(f"Config saved to: {save_path}transformer_config.json")

    @classmethod
    def load_model(cls, model_path='models/transformer_model.h5',
                   config_path='models/transformer_config.json'):
        """
...
def main():
    """
    Main execution function - complete Transformer training pipeline
    """
    print("\n" + "="*80)
    print("TRANSFORMER MODEL FOR S&P 500 PREDICTION")
    print("="*80 + "\n")
    ...
    transformer = TransformerPredictor(
        ...
        dense_units=[32],             # Dense layers after Transformer
    ...
    transformer.train(
        ...
        batch_size=32,
        patience=15,
        warmup_epochs=10,  # Learning rate warmup for Transformers
        verbose=1
    )
    ...
    train_metrics = transformer.evaluate(X_train, y_train, dataset_name='Training')
    val_metrics = transformer.evaluate(X_val, y_val, dataset_name='Validation')
    test_metrics = transformer.evaluate(X_test, y_test, dataset_name='Test')
    ...
    transformer.plot_predictions(test_metrics['actuals'], test_metrics['predictions'],
    ...
    print("  - results/transformer_results.json")
    print("  - plots/transformer_training_history.png")
    print("  - plots/transformer_predictions_test.png")
    print("\n" + "="*80 + "\n")
