"""
Enhanced Transformer Model for S&P 500 Prediction (V1.1)
=========================================================
This script implements an enhanced Transformer model with all major improvements
...
class EnhancedTransformerConfig:
    ...
    @classmethod
    def load(cls, filepath: Union[str, Path]) -> 'EnhancedTransformerConfig':
        filepath = Path(filepath)
...
class EnhancedTransformerPredictor:
    """
    Enhanced Transformer model for S&P 500 prediction with custom biases
    """

    def __init__(self, config: EnhancedTransformerConfig, n_features: int = 1):
        self.config = config
        ...
        logger.info("Enhanced Transformer Predictor initialized with custom biases")
    ...
    def prepare_data(self, features: pd.DataFrame) -> Tuple[Tuple[np.ndarray, np.ndarray, np.ndarray], ...]:
        """
        Prepare data for dual-output Transformer training (price + direction)
    ...
    def build_model(self):
        """
        Build enhanced Transformer model with dual outputs
        """
        print("\n" + "="*60)
        print("BUILDING ENHANCED TRANSFORMER MODEL (V1.1)")
        print("="*60)
    ...
    def train(self, train_data: Tuple[np.ndarray, np.ndarray, np.ndarray],
        ...
        """
        Train the enhanced Transformer model with learning rate warmup
        """
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                monitor='val_price_output_loss',
                mode='min',
                filepath='best_enhanced_transformer_model.keras',
                save_best_only=True,
                verbose=0
            ),
            lr_scheduler
    ...
    def evaluate(self, test_data: Tuple[np.ndarray, np.ndarray, np.ndarray],
        ...
        """
        Evaluate the enhanced Transformer model
        """
        ...
        logger.info("\n" + "="*60)
        logger.info("ENHANCED TRANSFORMER MODEL EVALUATION RESULTS")
        logger.info("="*60)
    ...
    def plot_training_history(self, save_path='plots/'):
        ...
        plt.tight_layout()
        plt.savefig(f'{save_path}enhanced_transformer_training_history.png', dpi=300, bbox_inches='tight')
        print(f"Training history plot saved to: {save_path}enhanced_transformer_training_history.png")
        plt.close()
    ...
    def plot_results(self, predictions: np.ndarray, actual: np.ndarray,
        ...
        plt.title(f'Enhanced Transformer Predictions - {dataset_name} Set', fontsize=16)
        plt.xlabel('Time Steps')
        plt.ylabel('Price ($)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(f'{save_path}enhanced_transformer_predictions_{dataset_name.lower()}.png',
                   dpi=300, bbox_inches='tight')
        print(f"Predictions plot saved to: {save_path}enhanced_transformer_predictions_{dataset_name.lower()}.png")
        plt.close()
    ...
    def save_model(self, save_path='models/'):
        ...
        self.model.save(f'{save_path}enhanced_transformer_model.keras')
        self.config.save(f'{save_path}enhanced_transformer_config.json')

        print(f"Model saved to: {save_path}enhanced_transformer_model.keras")
        print(f"Config saved to: {save_path}enhanced_transformer_config.json")




def create_default_config(sequence_length: int = 20) -> EnhancedTransformerConfig:
    """Create default configuration for enhanced Transformer"""
...
def main():
    """
    Main execution function - complete Enhanced Transformer V1.1 training pipeline
    ...
    """
    print("\n" + "="*80)
    print("ENHANCED TRANSFORMER MODEL V1.1 FOR S&P 500 PREDICTION")
    print("="*80 + "\n")
    ...
    config.save('models/enhanced_transformer_v1.1_config.json')

    logger.info("=" * 70)
    logger.info("📊 ENHANCED TRANSFORMER PREDICTOR V1.1")
    logger.info("=" * 70)
    ...
    logger.info(f"🧠 Transformer: {config.num_layers} layers, {config.num_heads} heads, d_model={config.d_model}")
    logger.info(f"🎯 Max Epochs: {config.epochs}")
    logger.info("=" * 70)

    # Initialize Enhanced Transformer predictor with V1.1 architecture
    transformer = EnhancedTransformerPredictor(config, n_features=metadata['n_features'])
    ...
    # Build enhanced model with multi-head attention and dual outputs
    ...
    transformer.train(
        ...
        warmup_epochs=10  # LR warmup for Transformer
    )
    ...
    train_metrics, _, _ = transformer.evaluate(
        (X_train, y_train, np.zeros_like(y_train)),
        None  # No features DataFrame needed
    )
    val_metrics, _, _ = transformer.evaluate(
        (X_val, y_val, np.zeros_like(y_val)),
        None
    )
    test_metrics, price_predictions, y_test_actual = transformer.evaluate(
    ...
    results = {
        ...
        'config': {
            ...
            'dff': transformer.config.dff,
            'dropout_rate': transformer.config.dropout_rate,
            'learning_rate': transformer.config.learning_rate,
            'growth_bias': transformer.config.growth_bias,
            'training_time': transformer.training_time
        }
    }

    with open('results/enhanced_transformer_v1.1_results.json', 'w') as f:
        json.dump(results, f, indent=4)

    print("\nResults saved to: results/enhanced_transformer_v1.1_results.json")
    ...
    transformer.plot_training_history(save_path='plots/')
    transformer.plot_results(price_predictions, y_test_actual, save_path='plots/')

    # Save model
    transformer.save_model(save_path='models/')

    print("\n" + "="*80)
    print("ENHANCED TRANSFORMER V1.1 PIPELINE COMPLETE")
    print("="*80)
    print("\nGenerated files:")
    print("  - models/enhanced_transformer_v1.1_model.h5")
    print("  - models/enhanced_transformer_v1.1_config.json")
    print("  - results/enhanced_transformer_v1.1_results.json")
    print("  - plots/enhanced_transformer_v1.1_training_history.png")
    print("  - plots/enhanced_transformer_v1.1_predictions_test.png")
    print("\n" + "="*80 + "\n")

    return transformer, test_metrics
