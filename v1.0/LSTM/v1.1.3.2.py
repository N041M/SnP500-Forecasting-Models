...
@dataclass
class ModelConfig:
    """Configuration with market bias parameters."""
    ...
    top_performer_weight: float = 1.4
    low_rep_boost: float = 1.2
    top_performer_percentile: float = 0.95
...
class SP500Predictor:
    ...
    def __init__(self, config: ModelConfig):
        ...
        # Extract all configuration parameters as instance variables
        self.csv_files = config.csv_files
        ...
        self.growth_bias = config.growth_bias
        self.top_performer_weight = config.top_performer_weight
        self.low_rep_boost = config.low_rep_boost
        self.top_performer_percentile = config.top_performer_percentile
        ...
        logger.info("Predictor initialized with custom biases")
        logger.info(f"Growth bias: {self.growth_bias}")
        logger.info(f"Top performer weight: {self.top_performer_weight}")
        logger.info(f"Low representation boost: {self.low_rep_boost}")
        logger.info(f"TensorFlow version: {tf.__version__}")
    
    def load_csv_data(self) -> pd.DataFrame:
        logger.info(f"Loading data from {len(self.csv_files)} CSV files...")
        try:
            raw_dataframes = []
    ...
    def _apply_sector_rotation(self, features: pd.DataFrame):
        ...
        if 'Market_Cap_Billions' in self.company_info.columns and 'Sector' in self.company_info.columns:
            ...
            # Apply sector rotation based on volatility
            if features['volatility'].iloc[-1] > features['volatility'].quantile(0.7):
                defensive_sectors = ['Utilities', 'Consumer Staples', 'Health Care']
                for sector in defensive_sectors:
                    if sector in sector_weights.index:
                        sector_weights[sector] *= self.low_rep_boost
        ...
        if self.is_aggregate:
            ...
            if 'Close' in data.columns:
                ...
                if len(returns) > 0:
                    ...
                    if momentum > 0.05:
                        self.momentum_adjustment = self.top_performer_weight
            ...
            if performances:
                threshold = np.percentile(list(performances.values()), 
                                         self.top_performer_percentile * 100)
                for company, perf in performances.items():
                    if perf >= threshold:
                        self.company_weights[company] = self.top_performer_weight
                        logger.info(f"  Top performer: {company} (weight: {self.top_performer_weight})")
                    else:
                        self.company_weights[company] = 1.0
    ...
    def prepare_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(features) < self.sequence_length + 10:
            logger.warning(f"Insufficient data after feature engineering. Need at least {self.sequence_length + 10} rows, got {len(features)}. Proceeding for testing.")
    ...
    def apply_growth_bias(self, predictions: np.ndarray, features: pd.DataFrame) -> np.ndarray:
        bias_factor = 1 + self.growth_bias
        logger.info("Applying standard growth bias")
        
        for i in range(len(predictions)):
            time_factor = 1 - np.exp(-i / 10)
            predictions[i] *= (1 + self.growth_bias * time_factor * bias_factor)
        return predictions
    ...
    def prepare_data(self, features: pd.DataFrame) -> Tuple[Tuple[np.ndarray, np.ndarray, np.ndarray], ...]:
        ...
        for i in range(self.sequence_length, len(dataset_scaled)):
            X.append(dataset_scaled[i-self.sequence_length:i, 0])
    ...
    def build_model(self, input_shape: Tuple[int,]) -> tf.keras.Model:
        ...
        x = tf.keras.layers.Bidirectional(
            tf.keras.layers.LSTM(self.lstm_units[0], return_sequences=len(self.lstm_units) > 1)
        )(inputs)
        ...
        growth_bias_value = self.growth_bias  # Capture for use in @tf.function
        ...
        def biased_mse(y_true, y_pred):
            ...
            bias_penalty = tf.where(diff < 0, 
                                   tf.abs(diff) * (1 + growth_bias_value),
                                   tf.abs(diff))
    ...
    def train(self, train_data: Tuple[np.ndarray, np.ndarray, np.ndarray], 
        ...
        logger.info("Starting training with biases...")
    ...
    def plot_results(self, predictions: np.ndarray, actual: np.ndarray, 
        ...
        trend = actual[-n_points] * (1 + self.growth_bias) ** (x / 252)
...
def create_default_config(csv_files: List[str], 
                         sequence_length: int = 20,
                         growth_bias: float = 0.005,
                         top_performer_weight: float = 1.8,
                         low_rep_boost: float = 1.5) -> ModelConfig:
    """
    Create default configuration with key parameters as arguments.
    
    Args:
        csv_files: List of CSV file paths
        sequence_length: Number of time steps for LSTM sequences
        growth_bias: Growth bias factor (default 0.005 = 0.5%)
        top_performer_weight: Weight for top performing assets
        low_rep_boost: Boost factor for low representation sectors
    """
    return ModelConfig(
        csv_files=csv_files,
        ...
        top_performer_weight=top_performer_weight,
        low_rep_boost=low_rep_boost,
        top_performer_percentile=0.95
    )
...
def main():
    ...
    # Create configuration using parameters
    ...
    logger.info("=" * 70)
    logger.info("📊 S&P 500 PREDICTOR CONFIGURATION")
    logger.info("=" * 70)
    logger.info(f"📁 CSV Files: {len(CSV_FILES)} files")
    logger.info(f"📈 Growth Bias: {GROWTH_BIAS:.3%}")
    logger.info(f"⭐ Top Performer Weight: {TOP_PERFORMER_WEIGHT}x")
    logger.info(f"📉 Low Rep Boost: {LOW_REP_BOOST}x")
    ...
    predictor = SP500Predictor(config)
    ...
    try:
        ...
        predictor.save_model('sp500_model.keras')
...
if __name__ == "__main__":
    """
    S&P 500 LSTM Predictor with Refactored Configuration (v2.1.0)
    ===================================================================
    
    Key Improvements in v2.1.0:
    - Configuration parameters extracted as instance variables in __init__
    - Single source of truth for configuration values in main()
    - Reduced repetitive self.config.parameter references throughout code
    - More maintainable and readable parameter access
    
    Configuration Parameters (defined once, used everywhere):
    - CSV_FILES: List of data file paths
    - SEQUENCE_LENGTH: LSTM sequence length (default: 20)
    - GROWTH_BIAS: Market growth assumption (default: 0.005 = 0.5%)
    - TOP_PERFORMER_WEIGHT: Weight for strong momentum (default: 1.8x)
    - LOW_REP_BOOST: Boost for defensive sectors (default: 1.5x)
    """
